...
import search from '@salesforce/apex/UserLookupController.search';
import getRecentlyViewed from '@salesforce/apex/UserLookupController.getRecentlyViewed';
...
export default class UserLookup extends LightningElement {
    ...
    handleLookupSelectionChange(event) {
        console.log('handleLookupSelectionChange called');
        this.checkForErrors();

        if (this.errors.length === 0) {
            this.notifyParentLwc('Success', event, 'success');
        }
    }
    ...
    notifyParentLwc(title, event, variant) {
        console.log('inside notifyParentLwc', event.currentTarget);
        ...
        console.log('Event dispatched 1.6');
    }
}